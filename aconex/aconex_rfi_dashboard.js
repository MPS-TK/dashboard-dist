...
(function () {
  ...
  var GDEF_FILE='aconex/global_defaults_rfi.json';
  var GDEF_CACHE='mps_aconex_gdef_rfi', GDEF_APPLIED='mps_aconex_gdefapplied_rfi';
  ...
  function gdefFactorySnap(){var sn=gdefSnapshot();var f=factoryCfg();sn.cols=JSON.parse(JSON.stringify(f.cols));sn.order=f.order.slice();sn.colorSchemes=normSchemes(f.colorSchemes);sn.fontScale=(f.fontScale!=null?f.fontScale:100);sn.padScale=(f.padScale!=null?f.padScale:20);sn.hdrFontSize=(f.hdrFontSize!=null?f.hdrFontSize:null);sn.hdrMaxLines=(f.hdrMaxLines||2);sn.fontSize=(f.fontSize||12);sn.rowPad=(f.rowPad!=null?f.rowPad:0);sn.wrap=!!f.wrap;sn.baseFont=(f.baseFont||DEF_BASEPX);sn.chartType=(f.chartType||'donut');return JSON.parse(JSON.stringify(sn));}
  ...
  function gdefPublish(changes){
    return gdefWhoAmI().then(function(who){
      ...
      var body={message:'Aconex RFI global defaults ('+who.by+')',content:btoa(unescape(encodeURIComponent(JSON.stringify({patches:list})))),branch:GH.branch};
  ...
  function gdefBody(){
    ...
    if(!S.gdefEditing){
      ...
      b.appendChild(el('div',{style:'display:flex;gap:8px;flex-wrap:wrap;align-items:center'},[el('button',{class:'btn',title:'Snapshot the current view, then change settings to stage a global default',onclick:function(){S.gdefBaseline=gdefSnapshot();S.gdefEditing=true;gdefRepaint();}},['Start editing']),el('button',{class:'btn',title:'Publish your CURRENT view (including column widths) as the team default. You will see exactly what changes before it saves.',onclick:function(){var c=gdefDiff(gdefFactorySnap());if(!Object.keys(c).length){toast('Your current view already matches the factory defaults \u2014 nothing to save.');return;}gdefConfirmDialog(c);}},['Save Current']),el('span',{class:'muted',style:'font-size:11px'},[gdefLastText()])]));
    } else {
      ...
      b.appendChild(el('div',{style:'display:flex;gap:8px;flex-wrap:wrap'},[saveBtn,el('button',{class:'btn',title:'Publish your CURRENT view (including column widths) as the team default. You will see exactly what changes before it saves.',onclick:function(){var c=gdefDiff(gdefFactorySnap());if(!Object.keys(c).length){toast('Your current view already matches the factory defaults \u2014 nothing to save.');return;}gdefConfirmDialog(c);}},['Save Current']),el('button',{class:'btn',title:'Stop editing without publishing',onclick:function(){S.gdefEditing=false;S.gdefBaseline=null;gdefRepaint();}},['Cancel'])]));
      try{clearInterval(S._gdefTimer);}catch(e){}
  ...
  function migrateOverrideKeys(){var byOld={};(S.allRows||[]).forEach(function(r){if(r&&r._seqNo!=null&&r.aconexRef)byOld[String(r._seqNo)]=r.aconexRef;});var ch=false;Object.keys(S.overrides||{}).forEach(function(k){if(/^\d+$/.test(k)&&byOld[k]){var tgt=byOld[k];var src=S.overrides[k]||{};var dst=S.overrides[tgt]||(S.overrides[tgt]={});Object.keys(src).forEach(function(f){if(dst[f]==null)dst[f]=src[f];});delete S.overrides[k];ch=true;}});if(ch)saveOverrides();}
  function restoreXData(){try{var m=JSON.parse(localStorage.getItem(('mps_aconex_rfi_xdata_'+CFG.mpsProjectNo))||'null');if(!m)return;delete m.__full;S.allRows.forEach(function(r){var d=m[normRef(r.aconexRef)];if(!d)return;r._mpsMails=d.mps||[];r._bhpMails=d.bhp||[];r._autoFu1=d.fu1||'';r._autoFu2=d.fu2||'';r._refMailId=d.rid||'';r._refMailbox=d.rbox||'';r._refPid=d.pid||'';if(d.p&&!r.mailNo)r.mailNo=d.p;if(d.q&&!r.respMailNo)r.respMailNo=d.q;});}catch(e){}}
  function initRows(){
    S.allRows=SEED.map(function(d,i){var r={};MANUAL_FIELDS.forEach(function(k){r[k]=(d[k]!=null?d[k]:'');});r.mailNo='';r.respMailNo='';r.type=refType(r.aconexRef);r.rfiNo=parseRefNo(r.aconexRef);r._rfiNoOrig=r.rfiNo;r._seqNo=i+1;return r;});
  ...
  function reconcileAconexStatus(){
    ...
    var i=0,changed=0;
    function worker(){if(i>=targets.length)return Promise.resolve();var r=targets[i++],pid=r._refPid||CFG.projectId;return det(pid,r._refMailId).then(function(el){var s=mailStatus(el).toLowerCase();if(s&&s.indexOf('clos')>=0){r.closed='Closed';if(!r.dateClosed&&r.dateRespRecd)r.dateClosed=r.dateRespRecd;var _o=S.overrides[rowKey(r)]||(S.overrides[rowKey(r)]={});_o.closed='Closed';if(r.dateClosed)_o.dateClosed=r.dateClosed;changed++;}}).then(worker);}
    var n=Math.min(4,targets.length),ws=[];for(var w=0;w<n;w++)ws.push(worker());
    return Promise.all(ws).then(function(){S._acStatusRunning=false;if(changed){saveOverrides();try{ghPush();}catch(e){}recomputeAuto();applyScope();renderStats();renderChart();renderDaysOpen();renderBody();}}).catch(function(){S._acStatusRunning=false;});
  }
  ...
  // ---- v12.17 filters: Date Sent range (a), hidden rows (b), Aconex Ref text (c) ----
  ...
  function applyScope(){
    S.rows=S.allRows.filter(function(r){
      ...
      if(isRowHidden(r))return false;            // feature (b) — hidden rows leave every calculation
      if(!passDateFilter(r))return false;        // feature (a) — Date Sent range
      if(!passRefFilter(r))return false;         // feature (c) — Aconex Reference No text
      return true;
    });
    applyFilters();
  }
  ...
  function donutFloorObs(){ if(donutFloorObs._p)return; donutFloorObs._p=true; requestAnimationFrame(function(){ donutFloorObs._p=false; try{applyDonutFloor();}catch(e){} }); }
  function applyDonutFloor(){
    ...
    // mps_ui_tweaks.js inserts its -/+ size steppers into #chartctl AFTER we render, which widens the
    // header; re-measure whenever the header controls change so the floor covers the final control set.
    try{var ctl=R.querySelector('.chartctl');
      if(ctl&&ctl!==applyDonutFloor._el){ if(applyDonutFloor._o)applyDonutFloor._o.disconnect(); applyDonutFloor._el=ctl; applyDonutFloor._o=new MutationObserver(donutFloorObs); applyDonutFloor._o.observe(ctl,{childList:true}); }
    }catch(e){}
  }
  function applySplit(){
    if(!root)return;var row=root.querySelector('.cgrow2');if(!row)return;
    ...
    if(S.chartSplit==null||!isFinite(+S.chartSplit)||Math.abs((+S.chartSplit)-0.5)<1e-6){L.style.flex='1 1 0%';L.style.maxWidth='none';L.style.minWidth='0';R.style.flex='0 0 '+floor+'px';R.style.maxWidth='none';R.style.minWidth=floor+'px';return;}var p=chartSplitPct();
    L.style.flex='0 1 calc('+(p*100).toFixed(2)+'% - 5px)';L.style.maxWidth='none';L.style.minWidth='0';
  ...
  /* ---- which RFIs/TQs the bar chart draws --------------------------------
     One outlier (a long-running RFI) flattens every other bar. Unticking it here
     drops it from the CHART only — the register, filters and STATS tiles keep it.
     Held in S.doHide, so the choice survives a reload. */
  function doHidden(){if(!S.doHide)S.doHide=[];return S.doHide;}
  function doKey(r){return (r.type||'RFI')+'|'+String(r.rfiNo==null?'':r.rfiNo).trim();}
  function doLabel(r){var isB=(String(r.sender||'').toUpperCase().indexOf('BHP')>=0)||/^BHP/i.test(r.aconexRef||'');return (isB?'BHP ':'')+(r.type||'RFI')+' ('+r.rfiNo+')';}
  function doBaseRows(){return S.filtered.filter(function(r){return !(S.doHideClosed&&isClosed(r));}).sort(function(a,b){var x=parseFloat(a.rfiNo),y=parseFloat(b.rfiNo);if(!isNaN(x)&&!isNaN(y))return x-y;return String(a.rfiNo)<String(b.rfiNo)?-1:1;});}
  function doShownRows(){var h=doHidden();return doBaseRows().filter(function(r){return h.indexOf(doKey(r))<0;});}
  function doItemsPanel(anchor){
    var wrapEl=root.getElementById('wrap');
    var ex=root.getElementById('dipanel');if(ex){ex.remove();return;}
    var panel=el('div',{id:'dipanel',class:'mfpanel',style:'min-width:210px;max-height:640px;overflow:auto'});
    var listWrap=el('div',{});
    var srch=el('input',{type:'search',placeholder:'\u2315 Filter items\u2026',style:'width:100%;box-sizing:border-box;font-size:11px;padding:2px 5px;border:1px solid #cfd8e3;border-radius:4px;margin-bottom:4px'});
    function place(){var a2=root.getElementById('dofilt')||anchor;if(!a2||!panel.isConnected)return;var ar=a2.getBoundingClientRect(),wr=wrapEl.getBoundingClientRect();panel.style.left=Math.max(4,Math.min(ar.left-wr.left,wr.width-panel.offsetWidth-6))+'px';panel.style.top=(ar.bottom-wr.top+4)+'px';}
    function apply(list){S.doHide=list;saveCfg();renderDaysOpen();place();}
    function build(){
      listWrap.innerHTML='';
      var q=(srch.value||'').toLowerCase(),h=doHidden(),all=doBaseRows();
      var shown=all.filter(function(r){return !q||(doLabel(r)+' '+(r.description||'')).toLowerCase().indexOf(q)>=0;});
      if(!shown.length){listWrap.appendChild(el('div',{class:'muted',style:'font-size:11px;padding:4px'},['No matches.']));return;}
      shown.forEach(function(r){
        var k=doKey(r),cb=el('input',{type:'checkbox'});cb.checked=h.indexOf(k)<0;
        cb.onchange=function(){var t=doHidden().slice(),i=t.indexOf(k);if(cb.checked){if(i>=0)t.splice(i,1);}else if(i<0)t.push(k);apply(t);};
        listWrap.appendChild(el('label',{class:'mfrow',title:r.description||''},[cb,el('span',{style:'flex:1'},[doLabel(r)])]));
      });
    }
    srch.oninput=function(){build();};
    panel.appendChild(el('div',{class:'mfhd'},[
      el('a',{title:'Show every RFI/TQ in the chart',onclick:function(){apply([]);build();}},['(All)']),
      el('a',{title:'Hide every RFI/TQ from the chart',onclick:function(){apply(doBaseRows().map(doKey));build();}},['(None)']),
      el('a',{title:'Swap what is shown and what is hidden',onclick:function(){var h=doHidden();apply(doBaseRows().map(doKey).filter(function(k){return h.indexOf(k)<0;}));build();}},['(Invert)']),
      el('a',{title:'Close',style:'margin-left:auto',onclick:function(){panel.remove();}},['\u2715'])
    ]));
    panel.appendChild(srch);build();panel.appendChild(listWrap);wrapEl.appendChild(panel);place();
  }
  function renderDaysOpen(){
    var box=root.getElementById('daysopen');if(!box)return;box.innerHTML='';
    var allRows=doBaseRows(),rows=doShownRows(),nHid=allRows.length-rows.length;
    var def=statDef();
  ...
  function renderChart(){
    ...
    if(ctl){ctl.innerHTML='';
      ...
      ctl.appendChild(btn('Autofit','Autofit \u2014 size the chart to fill the panel',function(){autoSizeChart(true);},'sq'));ctl.appendChild(el('span',{class:'muted',style:'font-size:11px;margin-left:4px'},['Size']));
  ...
  function autoSizeChart(force){
    ...
    requestAnimationFrame(function(){requestAnimationFrame(function(){
      try{
        ...
        if(!cp)return;var box=root.getElementById('chart');if(!box)return;if(cp.classList.contains('coll'))return;
        var rows=box.querySelectorAll('.chartsrow');if(!rows.length)return;var n=rows.length;
        // donuts sit SIDE BY SIDE: grow each to the smaller of its column width and the available height, less padding.
        var sBody=stats&&stats.querySelector('.cpbody');
        var availH=(sBody?sBody.clientHeight:0)||box.clientHeight||220;if(availH<90)availH=90;
        var cs=getComputedStyle(box),padX=(parseFloat(cs.paddingLeft)||0)+(parseFloat(cs.paddingRight)||0);
        var availW=(box.clientWidth||320)-padX;if(availW<120)availW=120;
        var perW=(availW-10*(n-1))/n;
        var pc0=box.querySelector('.pchart'),extra=0;
        if(pc0){var t=pc0.querySelector('.pctitle'),lg=pc0.querySelector('.plegend');extra=(t?t.offsetHeight:0)+(lg?lg.offsetHeight:0)+14;}
        var side=Math.max(70,Math.min(perW,availH-extra)-16);
        var slider=root.querySelector('.cgrow2 .cpt input[type=range]');
        var mn=slider?(+slider.min||70)/100:0.7,mx=slider?(+slider.max||240)/100:2.4;
        var sc=Math.max(mn,Math.min(mx,side/120));
        S.chartScale=sc;S._afScale=sc;S.chartAutoFit=false;saveCfg();
        if(slider)slider.value=String(Math.round(sc*100));
        renderChartBody();
      }catch(e){}
    });});
  }
  ...
  // ============================================================
  // v12.17 filter controls — Date Sent range, Aconex Ref text, Rows hide
  ...
  function updateFilterChrome(){try{
    paintActive(root.querySelector('th[data-k="dateSent"]'),dateFilterActive(),'th');
    paintActive(root.querySelector('th[data-k="aconexRef"]'),refFilterActive(),'th');
  ...
  // ---- feature (c): Aconex Reference No. substring filter (type e.g. MPSBE) ----
  function refFilterInput(){var inp=el('input',{type:'text',id:'reffltinp',title:'Filter Aconex Reference No — type any part (e.g. MPSBE) to isolate a project. Applies to every calculation. Persists between sessions.',placeholder:'⌕ e.g. MPSBE',value:S.fltRef||''});inp.oninput=function(){S.fltRef=inp.value;refreshFilters();};if(refFilterActive())paintActive(inp,true,'inp');return inp;}

  // ---- feature (a): Date Sent range filter (presets + custom From/To, like Procore) ----
  ...
  function dateFilterBtn(){var b=el('div',{class:'mfbtn',id:'datefltbtn',title:'Filter by Date Sent — quick presets or a custom From/To range. Applies to every calculation. Persists between sessions.'},[el('span',{class:'dcv'},[dateBtnLabel()]),el('span',{style:'margin-left:4px'},['▾'])]);b.onclick=function(e){e.stopPropagation();openDateFilter(b);};if(dateFilterActive())paintActive(b,true,'btn');return b;}
  function openDateFilter(anchor){
    ...
    panel.appendChild(el('div',{class:'mfhd'},[el('span',{style:'font-weight:700;color:'+NAVY+';font-size:11px'},['Date Sent filter']),el('a',{title:'Clear the date filter',style:'margin-left:auto',onclick:function(){S.fltDate={from:'',to:'',preset:''};refreshFilters();var p=root.getElementById('datepanel');if(p)p.remove();}},['Clear']),el('a',{title:'Close',onclick:function(){var p=root.getElementById('datepanel');if(p)p.remove();}},['✕'])]));
    ...
    DATE_PRESETS.forEach(function(p){
      ...
      var rb=el('input',{type:'radio',name:'mpsdtpreset'});
  ...
  // ---- feature (b): Rows button + hide/restore panel ----
  ...
  function renderRowsPanel(anchor){
    ...
    function rowLabel(r){return (r.type||'RFI')+' '+(r.rfiNo!=null?r.rfiNo:'')+' · '+(r.aconexRef||'')+(r.description?(' · '+r.description):'');}
    function build(){
      list.innerHTML='';
      var q=(srch.value||'').toLowerCase();
      var all=S.allRows.slice().sort(function(a,b){var x=parseFloat(a.rfiNo),y=parseFloat(b.rfiNo);if(!isNaN(x)&&!isNaN(y))return x-y;return 0;});
  ...
  function renderTable(){
    ...
    letr.appendChild(el('th',{class:'colc',title:'Count of the rows currently shown'},['#']));
    ...
    htr.appendChild(el('th',{class:'mps-cnt',style:'padding:1px 2px;text-align:center;color:'+NAVY,title:'Count — running number of the rows currently shown'},['#']));
    ...
    var ftr=el('tr',{class:'f'});ftr.appendChild(el('td',{class:'mps-selcell'},[]));ftr.appendChild(el('td',{class:'mps-cnt'},[]));
  ...
  function saveXDataMerge(updates){try{var m=JSON.parse(localStorage.getItem(('mps_aconex_rfi_xdata_'+CFG.mpsProjectNo))||'{}');for(var k in updates){m[k]=Object.assign(m[k]||{},updates[k]);}localStorage.setItem(('mps_aconex_rfi_xdata_'+CFG.mpsProjectNo),JSON.stringify(m));}catch(e){}}
...
async function fullScan(){
  ...
  function renderBody(){
    ...
    S.filtered.forEach(function(row,ri){
      ...
      tr.appendChild(el('td',{class:'mps-cnt',style:'text-align:center;color:#8a939b;padding:'+pad+'px 4px'},[String(ri+1)]));
      visKeys().forEach(function(k){
        var td,w=S.cols[k].w;
  ...
  function toggleNumDD(anchor){
    ...
    wrapEl.appendChild(panel);
    if(anchor){var ar=anchor.getBoundingClientRect();var vw=window.innerWidth,vh=window.innerHeight,pw=panel.offsetWidth||Math.round(vw*0.5);var top=Math.max(6,ar.bottom+2);panel.style.left=Math.max(6,Math.min(ar.left,vw-pw-8))+'px';panel.style.top=top+'px';panel.style.maxHeight=(vh-top-10)+'px';}else{panel.style.left='12px';panel.style.top='120px';}
  }
  ...
  function apvReloadProjectView(){
    ...
    if (c){
      ...
      S.selFilters = {}; S.statusSel = '__ALL__'; S.typeSel = []; S.fltDate = { from:'', to:'', preset:'' }; S.fltRef = ''; S.hiddenRows = []; S.colFilters = {}; S.globalSearch = '';
    }
    S._seenChecked = false;
  }
  ...
  function maybeAutoCrosscheck(){try{if(__xSyncing)return;var pid=S.xProjectId||detectProjectId();if(!pid)return;var c=loadXCache();var hasX=false;try{hasX=!!localStorage.getItem(('mps_aconex_rfi_xdata_'+CFG.mpsProjectNo));}catch(e){}if(hasX&&c&&c.ts&&(Date.now()-c.ts)<7200000)return;setTimeout(function(){crosscheckMail(null);},700);}catch(e){}}
