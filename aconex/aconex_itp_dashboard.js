...
(function () {
  ...
  var VERSION='v12.51', BUILD_DATE='8 Sep 2026';
  ...
  function factoryCfg(){
    ...
    return {order:FACTORY_ORDER.slice(),cols:cols,fontSize:12,rowPad:0,wrap:false,phases:DEFAULT_PHASES.map(function(p){return {id:p.id,label:p.label};}),chartType:'donut',hiddenDonuts:[],selFilters:{},selKnown:{},chartScale:1,colorSchemes:{status:{},lifecycleStatus:{},phase:{},toAction:{},dateRequired:{}},fgSchemes:{status:{},lifecycleStatus:{},phase:{},toAction:{},dateRequired:{}},fontFamily:'',baseFont:DEF_BASEPX,darkMode:false,collapsed:{},barExpanded:false,chartDataField:'status',fontScale:100,padScale:20,hpadScale:100,hdrFontSize:null,hdrMaxLines:2,colPri:{},colDefW:{},colNames:{},packageSel:null,delivSel:null,packageText:{},fltDate:{from:'',to:'',preset:''},fltRef:'',hiddenRows:[],seriesOrder:{},subFmt:'XXX-XX-XX'};
  }
  ...
  function mergeCfg(saved){var f=factoryCfg();var o=(saved.order||f.order).filter(function(k){return COLDEF[k];});FACTORY_ORDER.forEach(function(k){if(o.indexOf(k)<0)o.push(k);});o=o.filter(function(k){return k!=='rowNo';});o.unshift('rowNo');var cols={};o.forEach(function(k){var s=(saved.cols||{})[k]||{};cols[k]={show:s.show!=null?!!s.show:!!FACTORY_SHOW[k],w:s.w||COLDEF[k].w};});return {order:o,cols:cols,fontSize:saved.fontSize||12,rowPad:(saved.rowPad!=null?saved.rowPad:0),wrap:!!saved.wrap,phases:(saved.phases&&saved.phases.length?saved.phases:f.phases),chartType:saved.chartType||'donut',hiddenDonuts:saved.hiddenDonuts||[],selFilters:saved.selFilters||{},selKnown:saved.selKnown||{},chartScale:saved.chartScale||1,colorSchemes:normSchemes(saved.colorSchemes),fgSchemes:normFgSchemes(saved.fgSchemes),fontFamily:saved.fontFamily||'',baseFont:saved.baseFont||DEF_BASEPX,darkMode:!!saved.darkMode,collapsed:saved.collapsed||{},barExpanded:!!saved.barExpanded,chartDataField:saved.chartDataField||'status',fontScale:saved.fontScale||(saved.baseFont?Math.round(saved.baseFont/DEF_BASEPX*100):100),padScale:(saved.padScale!=null?saved.padScale:20),hpadScale:saved.hpadScale||100,hdrFontSize:(saved.hdrFontSize!=null?saved.hdrFontSize:null),hdrMaxLines:saved.hdrMaxLines||2,colPri:saved.colPri||{},colDefW:saved.colDefW||{},colNames:saved.colNames||{},packageSel:(saved.packageSel!=null?saved.packageSel:null),delivSel:(saved.delivSel!==undefined?saved.delivSel:null),packageText:saved.packageText||{},fltDate:(saved.fltDate&&typeof saved.fltDate==='object')?{from:saved.fltDate.from||'',to:saved.fltDate.to||'',preset:saved.fltDate.preset||''}:{from:'',to:'',preset:''},fltRef:saved.fltRef||'',hiddenRows:(saved.hiddenRows&&saved.hiddenRows.length?saved.hiddenRows.slice():[]),seriesOrder:(saved.seriesOrder&&typeof saved.seriesOrder==='object'?saved.seriesOrder:{}),subFmt:(saved.subFmt||'XXX-XX-XX'),sortKey:saved.sortKey||'',sortDir:(saved.sortDir===-1?-1:1),colFilters:(saved.colFilters&&typeof saved.colFilters==='object'?saved.colFilters:{}),globalSearch:saved.globalSearch||''};}
  ...
  function normFgSchemes(cs){cs=cs||{};return {status:cs.status||{},lifecycleStatus:cs.lifecycleStatus||{},phase:cs.phase||{},toAction:cs.toAction||{},dateRequired:cs.dateRequired||{}};}
  function saveCfg(){try{localStorage.setItem(LKEY,JSON.stringify({order:S.order,cols:S.cols,fontSize:S.fontSize,rowPad:S.rowPad,wrap:S.wrap,phases:S.phases,chartType:S.chartType,hiddenDonuts:S.hiddenDonuts,selFilters:S.selFilters,selKnown:S.selKnown,chartScale:S.chartScale,colorSchemes:S.colorSchemes,fgSchemes:S.fgSchemes,fontFamily:S.fontFamily,baseFont:S.baseFont,darkMode:S.darkMode,collapsed:S.collapsed,barExpanded:S.barExpanded,chartDataField:S.chartDataField,fontScale:S.fontScale,padScale:S.padScale,hpadScale:S.hpadScale,hdrFontSize:S.hdrFontSize,hdrMaxLines:S.hdrMaxLines,colPri:S.colPri,colDefW:S.colDefW,colNames:S.colNames,packageSel:S.packageSel,delivSel:S.delivSel,packageText:S.packageText,fltDate:S.fltDate,fltRef:S.fltRef,hiddenRows:S.hiddenRows,seriesOrder:S.seriesOrder,subFmt:S.subFmt,sortKey:S.sortKey,sortDir:S.sortDir,colFilters:S.colFilters,globalSearch:S.globalSearch}));}catch(e){}}
  ...
  // ---- per-project startup defaults (Deliverable Type(s) + Package Number(s)) ----
  function startupKey(){return 'mps_aconex_startup_'+CFG.projectId;}
  function loadStartup(){try{var j=JSON.parse(localStorage.getItem(startupKey())||'null');return (j&&typeof j==='object')?j:null;}catch(e){return null;}}
  function saveStartup(o){try{if(o)localStorage.setItem(startupKey(),JSON.stringify(o));else localStorage.removeItem(startupKey());}catch(e){}}
  function applyStartupDefaults(){
    var su=loadStartup();
    if(su&&su.deliverables!=null){
      if(su.deliverables==='__ALL__')S.delivSel=null;
      else{var dl=su.deliverables.filter(function(v){return S.deliverableTypes.indexOf(v)>=0;});S.delivSel=(dl.length>=S.deliverableTypes.length)?null:dl;}
    } else {
      var _dtd=loadDtDef();
      if(_dtd&&_dtd!=='__ALL__'&&S.deliverableTypes.indexOf(_dtd)>=0)S.delivSel=[_dtd];
      else if(S.delivSel){S.delivSel=S.delivSel.filter(function(v){return S.deliverableTypes.indexOf(v)>=0;});if(!S.delivSel.length)S.delivSel=null;}
    }
    if(su&&su.packages!=null){
      if(su.packages==='__ALL__')S.packageSel=null;
      else{var dp=distinctPackages();var pk=su.packages.filter(function(v){return dp.indexOf(v)>=0;});S.packageSel=(pk.length>=dp.length)?null:pk;}
    }
  }
  ...
  function openDelivPanel(anchor){
    ...
    var d=S.deliverableTypes.slice();
    var panel=el('div',{id:'dlvpanel',class:'mfpanel',style:'min-width:280px;max-width:440px'});
    function curSel(){var sx=S.delivSel;return sx==null?d.slice():sx.slice();}
  ...
  var C=loadCfg();
  var S={allRows:[],rows:[],filtered:[],loading:false,error:'',deliverableType:'__ALL__',deliverableTypes:[],seriesOrder:(C.seriesOrder||{}),subFmt:(C.subFmt||'XXX-XX-XX'),
         globalSearch:(C.globalSearch||''),colFilters:(C.colFilters&&typeof C.colFilters==='object'?C.colFilters:{}),sortKey:(C.sortKey||''),sortDir:(C.sortDir===-1?-1:1),
         order:C.order,cols:C.cols,fontSize:C.fontSize,rowPad:C.rowPad,wrap:C.wrap,
         phases:C.phases,chartType:C.chartType,hiddenDonuts:C.hiddenDonuts,selFilters:C.selFilters,selKnown:C.selKnown||{},chartScale:C.chartScale,colorSchemes:C.colorSchemes,fgSchemes:(C.fgSchemes||{status:{},lifecycleStatus:{},phase:{},toAction:{},dateRequired:{}}),
         ...
         packageSel:C.packageSel,delivSel:(C.delivSel!==undefined?C.delivSel:null),packageText:C.packageText||{},
  ...
  var GDEF_FILE='aconex/global_defaults_itp.json';
  var GDEF_CACHE='mps_aconex_gdef_itp', GDEF_APPLIED='mps_aconex_gdefapplied_itp';
  ...
  function gdefPublish(changes){
    return gdefWhoAmI().then(function(who){
      ...
      var body={message:'Aconex ITP global defaults ('+who.by+')',content:btoa(unescape(encodeURIComponent(JSON.stringify({patches:list})))),branch:GH.branch};
  ...
  function gdefConfirmDialog(changes){
    ...
    body.appendChild(el('div',{},['This updates the Doc. Register defaults for every user on the team. Your name and the time are recorded (shown below). Only these '+keys.length+' setting'+(keys.length===1?'':'s')+' change — nothing else each person has set is touched.']));
  ...
  function reparseSubsystems(){S.allRows.forEach(function(r){var d=parseTitle(r.title||'');r._subParsed=d.subsystem;if(d.subsystemName)r.subsystemName=d.subsystemName;var o=S.overrides[r.docNo]||{};if(!(o.subsystem!=null&&o.subsystem!==''))r.subsystem=(r._subParsed||'');});}
  function toggleSubFmt(anchor){
    var wrapEl=root.getElementById('wrap');var ex=root.getElementById('subfmtdd');if(ex){ex.remove();return;}
    var panel=el('div',{id:'subfmtdd',class:'panel',style:'min-width:230px'});
    panel.appendChild(el('h4',{style:'white-space:normal'},['Subsystem parse format']));
    panel.appendChild(el('div',{class:'muted',style:'font-size:11px;margin-bottom:6px;white-space:normal'},['How the Subsystem code is read from the document title. Manually typed values are kept and never re-parsed.']));
    ['XXX-XX-XX','XXXX-XX-XX','XXX-XX'].forEach(function(fmt){
      var cur=(S.subFmt||'XXX-XX-XX')===fmt;
      panel.appendChild(el('button',{class:'btn'+(cur?' alt':''),style:'display:block;width:100%;text-align:left;margin-bottom:4px'+(cur?';font-weight:700':''),onclick:function(){S.subFmt=fmt;saveCfg();reparseSubsystems();reconcileSelKnown();applyScope();renderTable();renderCharts();renderStats();var p=root.getElementById('subfmtdd');if(p)p.remove();toast('Subsystem parsed as '+fmt);}},[(cur?'✓ ':'')+fmt]));
    });
    panel.appendChild(el('button',{class:'btn',style:'margin-top:4px',onclick:function(){var p=root.getElementById('subfmtdd');if(p)p.remove();}},['Close']));
  ...
  function custFieldSelect(){
    ...
    var sel=el('select',{style:'box-sizing:border-box;height:32px;color:'+ACCENT+';border:2px solid '+ACCENT+';border-radius:5px;padding:0 5px;font-size:12px;font-weight:700;max-width:190px;background:#fff;cursor:pointer;vertical-align:middle',title:'Choose which Aconex field this column shows and filters (saved per project)'});
    flds.forEach(function(f){sel.appendChild(el('option',{value:f.id},[f.label]));});
    sel.value=S.customField||flds[0].id;
    sel.onchange=function(){pickCustField(sel.value);};try{requestAnimationFrame(function(){try{var tb=sel.closest('.toolbar');var ref=tb&&tb.querySelector('button.pkgbtn');if(ref&&ref.offsetHeight)sel.style.height=ref.offsetHeight+'px';}catch(e){}});}catch(e){}
    return sel;
  }
  ...
  // ---- v12.17 filters: Date Modified range (a), hidden rows (b), Document No text (c) ----
  ...
  function isRowHidden(r){return !!(S.hiddenRows&&S.hiddenRows.indexOf(selKeyOf(r))>=0);}
  function passDateFilter(r){var rg=fltDateRange();if(!rg)return true;var v=parseISODay(r.dateModified);if(v==null)return false;if(rg.from!=null&&v<rg.from)return false;if(rg.to!=null&&v>rg.to)return false;return true;}
  function passRefFilter(r){if(!refFilterActive())return true;return String(r.docNo||'').toLowerCase().indexOf(String(S.fltRef).toLowerCase().trim())>=0;}
  function refreshFilters(){applyScope();renderStats();renderCharts();renderBody();updateFilterChrome();saveCfg();}
  ...
  function updateFilterChrome(){try{
    paintActive(root.querySelector('th[data-k="dateModified"]'),dateFilterActive(),'th');
    paintActive(root.querySelector('th[data-k="docNo"]'),refFilterActive(),'th');
  ...
  function refFilterInput(){var inp=el('input',{type:'text',id:'reffltinp',title:'Filter Document No — type any part (e.g. a subsystem or discipline prefix) to isolate a project. Applies to every calculation. Persists between sessions.',placeholder:'⌕ Doc No…',value:S.fltRef||''});inp.oninput=function(){S.fltRef=inp.value;refreshFilters();};if(refFilterActive())paintActive(inp,true,'inp');return inp;}
  ...
  function dateFilterBtn(){var b=el('div',{class:'mfbtn',id:'datefltbtn',title:'Filter by Date Modified — quick presets or a custom From/To range. Applies to every calculation. Persists between sessions.'},[el('span',{class:'dcv'},[dateBtnLabel()]),el('span',{style:'margin-left:4px'},['▾'])]);b.onclick=function(e){e.stopPropagation();openDateFilter(b);};if(dateFilterActive())paintActive(b,true,'btn');return b;}
  function openDateFilter(anchor){
    ...
    panel.appendChild(el('div',{class:'mfhd'},[el('span',{style:'font-weight:700;color:'+NAVY+';font-size:11px'},['Date Modified filter']),el('a',{title:'Clear the date filter',style:'margin-left:auto',onclick:function(){S.fltDate={from:'',to:'',preset:''};refreshFilters();var p=root.getElementById('datepanel');if(p)p.remove();}},['Clear']),el('a',{title:'Close',onclick:function(){var p=root.getElementById('datepanel');if(p)p.remove();}},['✕'])]));
    ...
    DATE_PRESETS.forEach(function(p){
      ...
      var rb=el('input',{type:'radio',name:'mpsitpdtpreset'});
  ...
  function renderRowsPanel(anchor){
    ...
    function rowLabel(r){return (r.docNo||'(no doc no)')+(r.title?(' · '+r.title):'');}
    function build(){
      list.innerHTML='';
      var q=(srch.value||'').toLowerCase();
      var all=S.allRows.slice().sort(function(a,b){return String(a.docNo||'').localeCompare(String(b.docNo||''));});
  ...
  function fitRegisterHeight(){
    ...
    var rel=(tw.getBoundingClientRect().top-content.getBoundingClientRect().top)+content.scrollTop;
    var h=Math.max(180,Math.round(content.clientHeight-rel-8)); // fill from the table's top to the bottom of the visible content so the horizontal scrollbar stays at the viewport bottom regardless of outer scroll
  ...
  function CSS(){return '#wrap{position:fixed;inset:0;background:#f4f6f8;color:'+INK+';font:13px/1.4 "Segoe UI",Arial,sans-serif;display:flex;flex-direction:column}'
    ...
    +'.toolbar input[type=search],.toolbar select{border:1px solid #cfd8e3;border-radius:5px;padding:4px 8px;font-size:12px}.search{flex:1 1 120px;min-width:110px;max-width:240px}'
    ...
    +'.toolbar button.pkgbtn{font-weight:700;color:#0a58c2;border:2px solid #0a84ff;background:#eaf3ff;padding:4px 5px;box-shadow:0 0 0 2px rgba(10,132,255,.12)}.toolbar button.pkgbtn:hover{background:#dcebff;border-color:#0070e0}.dark .toolbar button.pkgbtn{color:#8ec5ff;background:#0e2438;border-color:#2f8bff}'
    ...
    +'#colpanel .crow .cpri{flex:0 0 auto;width:62px;text-align:center;font-size:11px;padding:2px 3px;border:1px solid #cfd8e3;border-radius:4px;box-sizing:border-box}'+'#colpanel .crow .cwid{flex:0 0 auto;width:62px;text-align:center;font-size:11px;padding:2px 3px;border:1px solid #cfd8e3;border-radius:4px;box-sizing:border-box}'+'#colpanel .crow .cwid:disabled{opacity:.5}'+'#colpanel .crow.chead{font-size:9px;font-weight:700;letter-spacing:.4px;color:#8a939b;padding-bottom:2px;border-bottom:1px solid #eef1f4;margin-bottom:2px}'+'#colpanel .crow.chead .chl{border:0;background:none;text-align:center}'+'.dark #colpanel .crow .cpri,.dark #colpanel .crow .cwid{background:#0e1621;color:#dfe7f0;border-color:#37485e}';}
  ...
  function renderAll(){
    ...
    wrap.appendChild(el('div',{class:'pbr'},[
      ...
      (S.capHit?el('span',{class:'pcap',title:'The Aconex register search returns at most 500 documents and cannot page beyond that. Set a Scope for this project (the ▾ button) to narrow it under 250.'},['⚠ first 250 shown · set a Scope']):null)
    ]));
    ...
    var toolbar=el('div',{class:'toolbar'},[
      ...
      (function(){var b=btn('Fit to 1 Page','Shrink every column so the whole register fits one page width (keeping headers to 2 lines)',function(){fitOnePage();});b.id='fitbtn';if(S._widthMode==='fit')b.classList.add('grn');return b;})(),
    ...
    var totalPanel=makeCPanel('total','TOTAL: '+delivTitle(),null,el('div',{class:'charts',id:'totalchart'}),'A single chart totalling all rows currently displayed. Click the title to roll this panel up.','TOTAL');
  ...
  function renderStats(){
    ...
    [tile(total,'Total',C('','#eaf1f8'),'Total documents in the selected Deliverable Type: '+delivTitle()),
  ...
  function renderCharts(){
    ...
    if(ctl){ctl.innerHTML='';
      ctl.appendChild(el('span',{class:'ccount',title:'Documents in the current Deliverable Type view'},[String(S.rows.length)+' · '+delivTitle()]));
  ...
  function isRowOpened(r){return !!(S.rowOpened&&S.rowOpened[selKeyOf(r)]);}
  function setRowOpened(r,v){S.rowOpened=S.rowOpened||{};if(v)S.rowOpened[selKeyOf(r)]=1;else delete S.rowOpened[selKeyOf(r)];}
  ...
  function updateSelBtn(){
    ...
    if(selBtnEl){
      ...
      if(pend){selBtnEl.removeAttribute('disabled');selBtnEl.setAttribute('title','Open the '+pend+' selected document'+(pend===1?'':'s')+' not yet opened — each in its own new tab, in the order they appear in the register');}
  ...
  function selCellTd(row,pad){
    ...
    var cb=el('input',{type:'checkbox',title:open?'Already opened in a new tab — still selected. Untick to clear the mark.':'Select this document for “Open Selected”'});
    ...
    cb.onchange=function(){
      ...
      if(!cb.checked){setRowOpened(row,false);cb.classList.remove('mps-opened');cb.setAttribute('title','Select this document for “Open Selected”');}
  ...
  function renderTable(){
    ...
    visKeys().forEach(function(k,vi){
      ...
      if(k==='status'||k==='lifecycleStatus'||k==='phase'||k==='toAction'||k==='dateRequired'){var pal=el('span',{class:'pal',title:'Change the '+cd.label+' colour scheme'},['🎨']);pal.onclick=function(ev){ev.stopPropagation();colorSchemePanel(k,pal);};hicons.push(pal);}if(k==='subsystem'){var sf=el('span',{class:'pal',title:'Choose how the Subsystem code is parsed from the title'},['⚙']);sf.onclick=function(ev){ev.stopPropagation();toggleSubFmt(sf);};hicons.push(sf);}if(hicons.length){thKids.push(el('span',{class:'hicons'},hicons));}
      var _fa=((k==='dateModified'&&dateFilterActive())||(k==='docNo'&&refFilterActive()));
      var th=el('th',{'data-k':k,style:'width:'+w+'px;min-width:'+Math.max(w,minHW(k))+'px;padding:'+Math.max(2,Math.round((S.rowPad-1)*(S.padScale||100)/100))+'px 2px;font-size:'+hdrFont()+'px'+(_fa?';background:#fff7f2;box-shadow:inset 0 -3px 0 '+ACCENT+';color:'+ACCENT:''),class:(COLDEF[k].edit?'mps-h':''),title:cd.tip},thKids);
    ...
    visKeys().forEach(function(k){
      ...
      if(k==='rowNo'){
        ...
        cell=el('td',{style:'width:'+S.cols[k].w+'px'},[dateFilterBtn()]);
      } else if(k==='docNo'){
        cell=el('td',{style:'width:'+S.cols[k].w+'px'},[refFilterInput()]);
  ...
  function renderColPanel(){
    ...
    var panel=el('div',{id:'colpanel',class:'panel',style:'left:12px;top:100px;max-height:calc(100vh - 128px);overflow:hidden'},[el('h4',{style:'white-space:normal'},['Columns']),el('div',{class:'muted',style:'font-size:11px;margin-bottom:6px'},[el('div',{style:'white-space:nowrap'},['• Tick to show/hide. ▲▼ reorder.']),el('div',{style:'white-space:nowrap'},['• Rename a header in its box (blank = default).']),el('div',{style:'white-space:nowrap'},['• PRIORITY is the Fit to 1 Page priority — 1 is served first, blanks rank equally.']),el('div',{style:'white-space:nowrap'},['• DEFAULT and CURRENT are the default and current widths in px.'])]),el('div',{class:'crow chead'},[el('span',{style:'flex:0 0 auto;width:46px'}),el('span',{style:'flex:0 0 auto;width:13px'}),el('span',{class:'cpri chl'},['PRIORITY']),el('span',{style:'flex:1;text-align:center'},['HEADER']),el('span',{class:'cwid chl'},['DEFAULT']),el('span',{class:'cwid chl'},['CURRENT'])])]);
    ...
    S.order.forEach(function(k,idx){
      ...
      var arrows=el('span',{style:'flex:0 0 auto;display:inline-flex;gap:3px'},[up,dn]);var pri=el('input',{type:'number',min:'1',step:'1',title:'Fit to 1 Page priority for '+colLabel(k)+' — 1 is served first. Leave blank to rank equally with the other blanks.',value:(colPri(k)==null?'':String(colPri(k))),class:'cpri'});pri.onchange=function(){setColPri(k,pri.value);pri.value=(colPri(k)==null?'':String(colPri(k)));};var dw=el('input',{type:'number',min:'8',step:'1',title:'Default width of '+colLabel(k)+' in px — what Reset Col Widths restores it to'+(isDateCol(k)?'. Date columns ignore this and use the measured width of the date control.':'.'),value:String(isDateCol(k)?dateColW():colDefW(k)),class:'cwid'});dw.disabled=isDateCol(k);dw.onchange=function(){setColDefW(k,dw.value);dw.value=String(colDefW(k));};var cw=el('input',{type:'number',min:'8',step:'1',title:'Current width of '+colLabel(k)+' in px',value:String(S.cols[k].w),class:'cwid'});cw.onchange=function(){var v=parseInt(cw.value,10);if(isNaN(v)||v<8){cw.value=String(S.cols[k].w);return;}S.cols[k].w=v;S.cols[k].userW=true;saveCfg();renderTable();};var rowEl=el('div',{class:'crow'},[arrows,cb,pri,nm,dw,cw]);
  ...
  function apvReloadProjectView(){
    // Per-project saved view: reload this project's own filters so nothing bleeds across projects.
    try { LKEY = 'mps_aconex_cfg_' + CFG.projectId; DKEY = 'mps_aconex_defcfg_' + CFG.projectId; } catch(e){}
    var c;
    try { c = loadCfg(); } catch(e){ c = null; }
    if (c){
      S.selFilters = c.selFilters || {};
      S.selKnown = c.selKnown || {};
      S.packageSel = (c.packageSel != null ? c.packageSel : null);
      S.delivSel = (c.delivSel !== undefined ? c.delivSel : null);
      S.packageText = c.packageText || {};
      ...
      S.colFilters = (c.colFilters && typeof c.colFilters === 'object') ? c.colFilters : {};
      S.globalSearch = c.globalSearch || '';
    } else {
      S.selFilters = {}; S.packageSel = null; S.delivSel = null; S.packageText = {}; S.fltDate = { from:'', to:'', preset:'' }; S.fltRef = ''; S.hiddenRows = [];
      S.colorSchemes = normSchemes(null); S.fgSchemes = normFgSchemes(null); S.seriesOrder = {}; S.colFilters = {}; S.globalSearch = '';
    }
  }
  function apvSelect(id, opts){
    ...
    if (opts.fetch !== false){
      ...
      try { S.overrides = loadOverrides(); } catch(e){}
      GH.path = 'aconex/overrides_' + CFG.projectId + '.json'; GH.sha = null;
      fetchData();
    }
  }
  function apvAutoSelect(){
    ...
    // set project + its scope + its saved view, without fetching yet (apvBoot fetches once)
    CFG.projectId = pick; CFG.projectName = apvName(pick); CFG.docScope = apvScopeFor(pick);
    try { localStorage.setItem(APV_SELKEY, pick); } catch(e){}
    apvReloadProjectView();
    try { S.overrides = loadOverrides(); } catch(e){}
    GH.path = 'aconex/overrides_' + CFG.projectId + '.json'; GH.sha = null;
    apvRenderDropdown();
  ...
  function apvBoot(){
    apvFetchProjects().then(function(list){
      ...
      if (!S.loading) fetchData(); else renderAll();
